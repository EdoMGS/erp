...
def calculate_project_costs(project):
    ...
    for nalog in project.radni_nalozi.all():
        for ang in nalog.angazmani.all():
            labor_costs += (
                ang.sati_rada * 
                ang.employee.calculate_total_salary()['base_salary']
            )
    ...
    if current_overhead:
        overhead_per_hour = (
            current_overhead.overhead_ukupno / 
            current_overhead.mjesecni_kapacitet_sati
        )
        total_hours = sum(
            ang.sati_rada 
            for nalog in project.radni_nalozi.all()
...
def update_project_financials(project):
    ...
    if project.financial_details.contracted_gross_price:
        project.financial_details.actual_profit = (
            project.financial_details.contracted_gross_price - 
            costs['total_costs']
        )
...
def process_completed_work_order(work_order):
    ...
    material_cost = sum(
        mat.materijal.cijena * mat.kolicina
        for mat in work_order.materijali_stavke.all()
    )
