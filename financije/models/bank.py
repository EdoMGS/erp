...
class BankTransaction(models.Model):
    ...
    bank_name = models.CharField(
        ...
        blank=True,
        null=True,
        verbose_name=_("Naziv banke")
    )
    tip_transakcije = models.CharField(
        max_length=10,
        choices=TRANSACTION_TYPES,
        verbose_name=_("Tip transakcije")
    )
    iznos = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        verbose_name=_("Iznos transakcije (€)")
    )
    ...
    referenca = models.CharField(
        max_length=100,
        unique=True,
        verbose_name=_("Referenca transakcije")
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True,
        verbose_name=_("Vrijeme evidentiranja u sustavu")
    )

    # Opcionalno, ako želiš označiti da je transakcija usklađena s knjigovodstvom
    is_reconciled = models.BooleanField(
        default=False,
        verbose_name=_("Usklađeno s knjigovodstvom?")
    )
