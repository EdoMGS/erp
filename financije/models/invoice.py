...
class Invoice(models.Model):
    ...
    issue_date = models.DateField(
        verbose_name=_("Datum izdavanja")
    )
    due_date = models.DateField(
        verbose_name=_("Datum dospijeća")
    )
    ...
    is_guaranteed = models.BooleanField(
        default=False,
        verbose_name=_("Garancija?")
    )
    guarantee_details = models.TextField(
        blank=True,
        null=True,
        verbose_name=_("Detalji garancije")
    )
    financial_guarantee = models.BooleanField(
        default=False,
        verbose_name=_("Financijska garancija?")
    )
    tender_statement = models.TextField(
        blank=True,
        null=True,
        verbose_name=_("Izjava za tender")
    )
...
class InvoiceLine(models.Model):
    ...
    tax_rate = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal('25.00'), 
                                 verbose_name=_("Stopa PDV-a"))
