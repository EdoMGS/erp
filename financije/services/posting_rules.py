...
def profit_share_distribution(*, tenant, base: Decimal, company: Decimal, workers: Decimal, owner: Decimal, rounding_diff: Decimal = Decimal("0.00"), idempotency_key: str | None = None):
    payload = {
        "base": base,
        "company": company,
        "workers": workers,
    ...
    return post_transaction(tenant=tenant, event="PROFIT_SHARE", payload=payload, idempotency_key=idempotency_key)
