...
def premjesti_artikl(request):
    if request.method == "POST":
        ...
        return JsonResponse(
            {"status": "success", "poruka": f"Artikl '{artikl.naziv}' uspješno premješten."}
        )
    ...
    if request.method == "POST":
        ...
        if artikl.kolicina >= kolicina:
            ...
            return JsonResponse(
                {
                    "status": "success",
                    "poruka": f"Rezervirano {kolicina} jedinica artikla '{artikl.naziv}'.",
                }
            )
        else:
            return JsonResponse(
                {"status": "error", "poruka": "Nema dovoljno zaliha za rezervaciju."}
            )
    return HttpResponse(status=405)
...
def provjeri_dostupnost(request):
    if request.method == "GET":
        ...
        return JsonResponse(
            {"naziv": artikl.naziv, "kolicina": artikl.kolicina, "status": artikl.status}
        )
    return HttpResponse(status=405)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            if korisnik:
                DnevnikDogadaja.objects.create(
                    korisnik=korisnik, poruka="Testiranje unosa u dnevnik", tip="info"
                )
