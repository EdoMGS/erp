...
class ArtiklSerializer(serializers.ModelSerializer):
    class Meta:
        model = Artikl
        fields = [
            'id', 'naziv', 'sifra', 'trenutna_kolicina', 'min_kolicina',
            'nabavna_cijena', 'prodajna_cijena', 'jedinica_mjere', 'lokacija',
            'kategorija', 'dobavljac', 'is_active'
        ]
...
class MaterijalSerializer(serializers.ModelSerializer):
    class Meta:
        model = Materijal
        fields = [
            'id', 'artikl', 'radni_nalog', 'status', 'tehnicki_nacrt', 'narudzbenica'
        ]
