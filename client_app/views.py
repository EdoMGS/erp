...
class InvoiceListView(BaseListView):
    ...
    def get_search_filter(self, query):
        return Q(invoice_number__icontains=query) | Q(client__name__icontains(query))  # Fixed parenthesis
...
def client_app_home(request):
    ...
    context = {
        'recent_clients': ClientSupplier.objects.filter(
            is_supplier=False
...
class OpportunityDeleteView(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if obj.client:
            log_client_activity(
                obj.client, 
                f"Obrisana prodajna prilika: {obj.name}", 
                'delete'
            )
        return super().delete(request, *args, **kwargs)
