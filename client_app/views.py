...
def opportunity_status_api(request, opportunity_id):
    try:
        opportunity = SalesOpportunity.objects.select_related("client").get(
            pk=opportunity_id
        )
        data = {
...
def client_app_home(request):
    ...
    context = {
        ...
        "recent_activities": ClientActivityLog.objects.select_related(
            "client"
        ...
        "today_activities": ClientActivityLog.objects.filter(
...
class SalesOrderListCreateAPIView(generics.ListCreateAPIView):
    ...
    serializer_class = (
        SalesOrderSerializer  # Ensure SalesOrderSerializer is defined for SalesOrder
    )
...
class OpportunityDeleteView(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if obj.client:
            log_client_activity(
                obj.client, f"Obrisana prodajna prilika: {obj.name}", "delete"
            )
        return super().delete(request, *args, **kwargs)
...
def opportunity_list(request):
    ...
    opportunities = SalesOpportunity.objects.select_related("client").order_by(
        "-created_at"
    )
