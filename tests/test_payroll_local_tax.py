...
def test_rijeka_7k_base(tenant, settings):
    ...
    emp = DummyEmp("RIJEKA")
    comp = compute_local_income_tax(
        tenant=tenant, employee=emp, payday=date(2025, 6, 1), taxable_base_eur=Decimal("7000")
    )
...
def test_zagreb_cutover_before_after(tenant, settings):
    ...
    settings.MONTHLY_BRACKET_THRESHOLD_EUR = 5000
    emp = DummyEmp("ZAGREB")
    comp_before = compute_local_income_tax(
        tenant=tenant, employee=emp, payday=date(2025, 2, 20), taxable_base_eur=Decimal("6000")
    )
    assert comp_before.tax_total == Decimal("1300.00")
    comp_after = compute_local_income_tax(
        tenant=tenant, employee=emp, payday=date(2025, 3, 5), taxable_base_eur=Decimal("6000")
    )
