...
def test_reports_compute_expected_values():
    ...
    Account.objects.create(tenant=tenant, number="471", name="VAT Receivable", account_type="active")
    Account.objects.create(tenant=tenant, number="500", name="Expense", account_type="expense")

    post_transaction(
        tenant=tenant,
        event="SALE_INVOICE_POSTED",
        payload={"net": Decimal("100"), "vat": Decimal("0"), "description": "s1", "date": date(2024, 4, 1)},
    )
    post_transaction(
        tenant=tenant,
        event="SALE_INVOICE_POSTED",
        payload={"net": Decimal("50"), "vat": Decimal("0"), "description": "s2", "date": date(2024, 2, 1)},
    )
    post_transaction(
        tenant=tenant,
        event="PURCHASE_INVOICE_POSTED",
        payload={"net": Decimal("40"), "vat": Decimal("0"), "description": "p1", "date": date(2024, 4, 15)},
    )

    pl = pnl_light(tenant=tenant)
    assert pl == {"revenue": Decimal("150.00"), "expense": Decimal("40.00"), "profit": Decimal("110.00")}

    bs = balance_sheet_light(tenant=tenant)
    assert bs == {"assets": Decimal("150.00"), "liabilities": Decimal("40.00"), "equity": Decimal("110.00")}
