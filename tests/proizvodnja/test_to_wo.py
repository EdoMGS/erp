...
def test_to_wo_idempotent():
    ...
    lot = PaintLot.objects.create(
        tenant=tenant, color="red", received_at=date(2024, 1, 1), remaining_qty=Decimal("10")
    )
...
def test_fefo_reservation_uses_oldest_lots_first():
    ...
    old_lot = PaintLot.objects.create(
        tenant=tenant, color="blue", received_at=date(2024, 1, 1), remaining_qty=Decimal("3")
    )
    new_lot = PaintLot.objects.create(
        tenant=tenant, color="blue", received_at=date(2024, 2, 1), remaining_qty=Decimal("5")
    )
