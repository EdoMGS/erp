...
class SalesOpportunity(models.Model):
    ...
    public_tender_number = models.CharField(max_length=100, blank=True, null=True,
                                            verbose_name=_("Broj javnog natječaja"))
...
class SalesContract(models.Model):
    ...
    status = models.CharField(max_length=50, choices=STATUS_CHOICES, default='draft', verbose_name=_("Status ugovora"))
...
class TenderPreparationManager(models.Manager):
    ...
    def pending_submissions(self):
        return self.filter(
            ...
            delivery_opening_datetime__gt=timezone.now()
        )
...
class TenderPreparation(BaseModel):
    ...
    tender_instructions = models.TextField(
        blank=True,
        null=True,
        verbose_name=_("Upute za ponuditelje")
    )
    evaluation_criteria = models.JSONField(
        blank=True,
        null=True,
        verbose_name=_("Kriteriji evaluacije")
    )
    tender_validity = models.DateField(
        blank=True,
        null=True,
        verbose_name=_("Rok valjanosti ponude")
    )
    delivery_opening_datetime = models.DateTimeField(
        blank=True,
        null=True,
        verbose_name=_("Datum i vrijeme otvaranja ponuda")
    )
    payment_terms = models.TextField(
        blank=True,
        null=True,
        verbose_name=_("Uvjeti plaćanja")
    )
    warranty_terms = models.TextField(
        blank=True,
        null=True,
        verbose_name=_("Uvjeti jamstva")
    )
    ...
    cashflow_plan = models.JSONField(
        blank=True,
        null=True,
        verbose_name=_("Plan cashflow-a")
    )
    nabava_updates = models.JSONField(
        blank=True,
        null=True,
        verbose_name=_("Ažurirane ponude materijala iz nabave")
    )
    notes = models.TextField(
        blank=True,
        null=True,
        verbose_name=_("Dodatne napomene")
    )
    status = models.CharField(
        max_length=50,
        choices=STATUS_CHOICES,
    ...
    required_documents = models.JSONField(
        default=dict,
        blank=True,
        verbose_name=_("Obavezna dokumentacija")
    )
    technical_specs = models.JSONField(
        default=dict,
        blank=True,
        verbose_name=_("Tehničke specifikacije")
    )
    ...
    project_manager = models.CharField(
        ...
        null=True,
        blank=True,
        verbose_name=_("Voditelj projekta (ime)")
    )
    ...
    estimated_production_time = models.DurationField(
        null=True,
        blank=True,
        verbose_name=_("Procijenjeno vrijeme proizvodnje")
    )
    ...
    def calculate_available_work_hours(self):
        ...
        """
        if self.production_capacity and hasattr(self.production_capacity, 'worker_count') and hasattr(self.production_capacity, 'monthly_hours'):
...
class TenderMaterial(models.Model):
    ...
    tax = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal('0.00'), verbose_name=_("PDV (%)"))
    total_price = models.DecimalField(max_digits=12, decimal_places=2, verbose_name=_("Ukupna cijena"), blank=True, null=True)
...
class TenderLabor(models.Model):
    ...
    total_cost = models.DecimalField(max_digits=12, decimal_places=2, verbose_name=_("Ukupni trošak"), blank=True, null=True)
...
class TenderSummary(models.Model):
    ...
    def calculate_final_price(self):
        self.final_price = (self.total_material_cost + self.total_labor_cost +
                            self.total_direct_cost + self.total_indirect_cost)
...
class TenderNeposredniTroskovi(BaseModel):
    tender = models.ForeignKey(TenderPreparation, on_delete=models.CASCADE, related_name="tender_neposredni_troskovi")
...
class TenderPosredniTroskovi(BaseModel):
    tender = models.ForeignKey(TenderPreparation, on_delete=models.CASCADE, related_name="tender_posredni_troskovi")
    description = models.CharField(max_length=255, verbose_name=_("Opis troška"))
    monthly_cost = models.DecimalField(max_digits=12, decimal_places=2, verbose_name=_("Mjesečni trošak"))
    project_cost = models.DecimalField(max_digits=12, decimal_places=2, verbose_name=_("Trošak po projektu"))
