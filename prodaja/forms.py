...
class FieldVisitForm(ModelForm):
    class Meta:
        model = FieldVisit
        fields = ['opportunity', 'visited_client', 'date_of_visit', 'purpose', 'created_by']
        widgets = {
...
class TenderPreparationForm(ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'tender_validity': forms.DateInput(
                attrs={'type': 'date'}
            ),
            ...
            'cashflow_plan': forms.Textarea(attrs={
                'rows': 4,
                'placeholder': 'Unesite plan cashflow-a u JSON formatu'
            }),
            'nabava_updates': forms.Textarea(attrs={
                'rows': 4,
            ...
            'payment_schedule': forms.Textarea(attrs={
                'rows': 4,
                'placeholder': 'Unesite plan plaćanja kao JSON'
            }),
...
TenderRasclambaFormSet = inlineformset_factory(
    TenderPreparation,
    TenderRasclamba,
    fields=('naziv_stavke', 'sati_rada', 'nas_materijal', 'vanjska_usluga', 'oprema_dijelovi', 'dobavljac'),
    extra=1,
    can_delete=True
)
