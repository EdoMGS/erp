...
class DodajTehnickuDokumentacijuView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        try:
            ...
            messages.error(self.request, f"Greška prilikom dodavanja tehničke dokumentacije: {str(e)}")
    ...
    def get_success_url(self):
        ...
        if radni_nalog_id:
            return reverse_lazy('lista_tehnicke_dokumentacije', kwargs={'radni_nalog_id': radni_nalog_id})
        if projekt_id:
...
class AzurirajTehnickuDokumentacijuView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        try:
            ...
            messages.error(self.request, f"Greška prilikom ažuriranja tehničke dokumentacije: {str(e)}")
            return super().form_invalid(form)
