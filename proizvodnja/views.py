...
def projekt_add(request):
    ...
    if request.method == 'POST':
        ...
        form = ProjektForm()
    return render(request, 'proizvodnja/dodaj_uredi_projekt.html', {
        'form': form, 
        'action': 'create',
...
def lista_projekata(request):
    ...
    return render(request, 'proizvodnja/lista_projekata.html', {
        'projekti': projekti,
        'current_naziv': naziv,
...
def lista_radnih_naloga(request, projekt_id):
    ...
    return render(request, 'proizvodnja/lista_radnih_naloga.html', 
...
def lista_tehnicke_dokumentacije(request):
    ...
    return render(request, 'proizvodnja/lista_tehnicke_dokumentacije.html', 
...
def radni_nalog_add(request):
    ...
    if request.method == 'POST':
        ...
        form = RadniNalogForm()
    return render(request, 'proizvodnja/radni_nalog_form.html', 
...
def univerzalni_radni_nalog(request, pk=None, action='create'):
    ...
    MaterialFormSet = inlineformset_factory(RadniNalog, RadniNalogMaterijal, 
                                          form=RadniNalogMaterijalForm, extra=1)
    VideoMaterijalFormSet = inlineformset_factory(RadniNalog, VideoMaterijal, 
                                                 form=VideoMaterijalForm, extra=1)
    VideoPitanjeFormSet = inlineformset_factory(RadniNalog, VideoPitanje, 
                                               form=VideoPitanjeForm, extra=1)
    OcjenaKvaliteteFormSet = inlineformset_factory(RadniNalog, OcjenaKvalitete, 
                                                  form=OcjenaKvaliteteForm, extra=1)
    AngazmanFormSet = inlineformset_factory(RadniNalog, Angazman, 
                                           form=AngazmanForm, extra=1)
    ...
    if request.method == 'POST' and action != 'view':
        ...
        if (form.is_valid() and materijal_formset.is_valid() and 
            video_materijal_formset.is_valid() and video_pitanje_formset.is_valid() and
            ocjena_kvalitete_formset.is_valid() and angazman_formset.is_valid()):
