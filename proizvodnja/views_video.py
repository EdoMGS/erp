...
class ListaVideoMaterijalaView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    template_name = (
        "tehnicke_dokumentacije/lista_tehnicke_dokumentacije.html"  # Provjerite putanju
    )
    ...
    def get_queryset(self):
        ...
        queryset = VideoMaterijal.objects.select_related("projekt").filter(
            is_active=True
        )
        if projekt_id:
...
class DodajVideoMaterijalView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        try:
            ...
            informiraj_korisnika(
                video_materijal
            )  # Provjerite implementaciju ove funkcije

            messages.success(self.request, "Video materijal uspješno dodan!")
            return response
        except Exception as e:
            messages.error(
                self.request, f"Greška prilikom dodavanja video materijala: {str(e)}"
            )
    ...
    def get_success_url(self):
        ...
        if projekt_id:
            return reverse_lazy(
                "lista_video_materijala", kwargs={"projekt_id": projekt_id}
            )
...
class AzurirajVideoMaterijalView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        try:
            ...
            messages.error(
                self.request, f"Greška prilikom ažuriranja video materijala: {str(e)}"
            )
            return self.form_invalid(form)
...
class ObrisiVideoMaterijalView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        try:
            ...
            messages.error(
                request, f"Greška prilikom brisanja video materijala: {str(e)}"
            )
...
class DodajVideoPitanjeView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        try:
            ...
            radni_nalog_id = self.kwargs.get("radni_nalog_id", None)
            if radni_nalog_id:
                video_pitanje.radni_nalog = get_object_or_404(
                    RadniNalog, id=radni_nalog_id
                )
            ...
            messages.error(
                self.request, f"Greška prilikom dodavanja video pitanja: {str(e)}"
            )
            return self.form_invalid(form)
    ...
    def get_success_url(self):
        ...
        if radni_nalog_id:
            return reverse_lazy(
                "lista_video_pitanja", kwargs={"radni_nalog_id": radni_nalog_id}
            )
