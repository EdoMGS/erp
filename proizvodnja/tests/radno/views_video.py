...
class DodajVideoPitanjeView(LoginRequiredMixin, CreateView):
    ...
    template_name = 'video_pitanja/dodaj_video_pitanje.html'
            ...
            messages.success(self.request, "Uspješno ažurirano!")
            return super().form_valid(form)
        video_pitanje.radni_nalog = get_object_or_404(RadniNalog, id=self.kwargs.get('radni_nalog_id'))
        video_pitanje.save()
    ...
    def get_success_url(self):
        return reverse_lazy('lista_video_pitanja', kwargs={'radni_nalog_id': self.kwargs['radni_nalog_id']})
