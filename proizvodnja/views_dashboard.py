...
class DashboardView(LoginRequiredMixin, View):
    ...
    def get_director_context(self):
        ...
        notifikacije = Notifikacija.objects.filter(korisnik=self.request.user, procitano=False).order_by('-created_at')[:5]
    ...
    def get_radnik_context(self, radnik):
        radni_nalozi = RadniNalog.objects.filter(
            Q(zadužena_osoba=radnik) | Q(dodatne_osobe=radnik),
            is_active=True
        ).select_related('projekt', 'grupa_posla').distinct()
    ...
    def get_financial_material_context(self):
        ...
        all_materijali = Materijal.objects.filter(is_active=True)  # Removed select_related
        ...
        total_stvarni_troskovi = sum(p.stvarni_troskovi for p in projekti if p.stvarni_troskovi)
    ...
    def get_filtered_radni_nalozi(self, projekt_id=None, radnik_id=None):
        ...
        qs = RadniNalog.objects.filter(is_active=True).select_related('projekt', 'grupa_posla')
        if projekt_id:
            qs = qs.filter(projekt_id=projekt_id)
        if radnik_id:
            # radnik može biti zaduzena_osoba ili u dodatne_osobe (M2M)
            qs = qs.filter(Q(zaduzena_osoba_id=radnik_id) | Q(dodatne_osobe__id=radnik_id)).distinct()  # Corrected field name
    ...
    def get_gantt_data(self, radni_nalozi):
        ...
        for nalog in radni_nalozi:
            ...
            if nalog.datum_zavrsetka:
                end_str = nalog.datum_zavrsetka.strftime('%Y-%m-%d')
            progress_val = float(nalog.postotak_napretka) / 100.0
            task_text = f"[{nalog.projekt.naziv_projekta}] {nalog.naziv_naloga}" if nalog.projekt else nalog.naziv_naloga
    ...
    def get_workload_data(self):
        ...
        for emp in employees:
            total_hours = Angazman.objects.filter(zaposlenik=emp, is_active=True).aggregate(
                total=Sum('sati_rada')
            )['total'] or 0
            data_list.append({
...
def api_dashboard_data(request):
    ...
    if korisnik.is_superuser:
        ...
        if position_title in ['Direktor', 'Voditelj']:
            ...
            # Radnik ili drugi
            radni_nalozi = RadniNalog.objects.filter(
                Q(zadužena_osoba=employee) | Q(dodatne_osobe=employee),
                is_active=True
            ).distinct()
            ukupni_nalozi = radni_nalozi.count()
            zavrseni_nalozi = radni_nalozi.filter(status='ZAVRSENO').count()
...
def get_worker_context(employee):
    ...
    return {
        ...
        'completed_tasks': RadniNalog.objects.filter(employee=employee, status='ZAVRSENO'),
    }
